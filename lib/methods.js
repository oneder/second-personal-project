	...
	'handleSoundEvent': function (roomId) {
		...
		if(currentRoom){
			if(currentRoom.queue.length > 0){
				...
				//currentRoom.tools[refIndex].gain = first.gain;
				//currentRoom.tools[refIndex].frequency = first.region;
				//currentRoom.tools[refIndex].pan = first.panAmount;
				Rooms.update(roomId, {$push: {tools: {$each: [newTool], $position: 1}}});
				Rooms.update(roomId, {$pop: {tools: -1}});
